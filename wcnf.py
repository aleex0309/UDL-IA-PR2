...
class WCNFFormula(object):
    ...
    def to_13wpm(self):
        ...
        hard_clauses_generated = []

        # soft clauses
        for soft in self.soft:
            if len(soft[1]) != 1: # not a 1-literal clause
                reification_var = formula13.new_var()
                generated[reification_var] = soft # save the soft clause
                hard_clauses_generated.append(soft[1] + [reification_var]) # add the new hard clause
                formula13.add_clause([-reification_var], soft[0]) # add the new soft clause
            else:
                formula13.add_clause(soft[1], soft[0]) # add the soft clause

        for hard_clause in self.hard:
            formula13.hard_clauses_to_13wpm(hard_clause)

        for new_hard in hard_clauses_generated:
            formula13.hard_clauses_to_13wpm(new_hard)

        return formula13
        # raise NotImplementedError("Your code here")

    def hard_clauses_to_13wpm(self, hard_clause):
        if len(hard_clause) < 3:
            self.add_clause((hard_clause * 3)[:3], TOP_WEIGHT)
        elif len(hard_clause) > 3:
            last = self.new_var()
            self.add_clause(hard_clause[:2] + [last], TOP_WEIGHT)
            self.hard_clauses_to_13wpm([-last]+hard_clause[2:])
        else:
